...
var _NArray_instances,
  _a,
  _NArray_arr,
  _NArray_computedShape,
  _NArray_length,
  _NArray_computedStrides,
  _NArray_ndim,
  _NArray_computeShape,
  _NArray_computeStrides,
  _NArray_get,
  _NArray_flatten;
...
class NArray {
  ...
  constructor(obj) {
    ...
    if (obj instanceof Array) {
      ...
      __classPrivateFieldSet(
        this,
        _NArray_arr,
        Array.from(__classPrivateFieldGet(this, _NArray_arr, "f")),
        "f",
      );
      __classPrivateFieldSet(
        this,
        ...
        __classPrivateFieldGet(
          ...
          "m",
          _NArray_computeShape,
        ).call(this, __classPrivateFieldGet(this, _NArray_arr, "f")),
        "f",
      );
      __classPrivateFieldSet(
        this,
        ...
        __classPrivateFieldGet(
          ...
          this,
          ...__classPrivateFieldGet(this, _NArray_computedShape, "f"),
        ),
        "f",
      );
      __classPrivateFieldSet(
        this,
        _NArray_ndim,
        __classPrivateFieldGet(this, _NArray_computedShape, "f").length,
        "f",
      );
  ...
  get length() {
    if (!__classPrivateFieldGet(this, _NArray_length, "f")) {
      __classPrivateFieldSet(
        this,
        _NArray_length,
        __classPrivateFieldGet(this, _NArray_arr, "f").length,
        "f",
      );
    }
  ...
  reduce(func) {
    let f = __classPrivateFieldGet(this, _NArray_arr, "f").reduce((a, b) =>
      func(a, b),
    );
    return f;
  }
  map(func) {
    let f = __classPrivateFieldGet(this, _NArray_arr, "f").map((e, i) =>
      func(e, i),
    );
    return new _a(f).reshape(...this.shape);
  }
  forEach(func) {
    __classPrivateFieldGet(this, _NArray_arr, "f").forEach((e, i) =>
      func(e, i),
    );
  }
  max() {
    let maxIndex = 0,
      maxElem = __classPrivateFieldGet(this, _NArray_arr, "f")[0];
  ...
  diag() {
    ...
    if (this.ndim === 1) {
      ...
      for (let i = 1; i < smaller; i++) {
        final[i] = __classPrivateFieldGet(this, _NArray_arr, "f")[
          this.shape[1] * i + i
        ];
      }
      return new _a(final);
  ...
  dot(y) {
    ...
    const shape1Last = tempShape1[tempShape1.length - 1],
      ...
      newLen = _a.calcNoOfElems(...newShape),
  ...
  reshape(...shape) {
    ...
    this.strides = __classPrivateFieldGet(
      this,
      _NArray_instances,
      "m",
      _NArray_computeStrides,
  ...
  toString() {
    if (this.length > globalThis.NArray_printThreshold) {
      ...
      finalStr += "...";
      finalStr += __classPrivateFieldGet(this, _NArray_arr, "f")[
        this.length - 1
      ];
      for (let i = 0; i < this.shape.length; i++) {
        finalStr += "]";
...
(_a = NArray),
  (_NArray_arr = new WeakMap()),
  (_NArray_computedShape = new WeakMap()),
  (_NArray_length = new WeakMap()),
  (_NArray_computedStrides = new WeakMap()),
  (_NArray_ndim = new WeakMap()),
  (_NArray_instances = new WeakSet()),
  (_NArray_computeShape = function _NArray_computeShape(x) {
    let size = [];
    ...
    if (path.length <= this.strides.length) {
      if (path.length === this.strides.length) {
        ...
        for (let i = 0; i < currShape; i++) {
          final[i] = __classPrivateFieldGet(
            this,
            _NArray_instances,
            "m",
            _NArray_get,
  ...
  (_NArray_flatten = function _NArray_flatten(
    x = __classPrivateFieldGet(this, _NArray_arr, "f"),
  ) {
    let final = [];
    let temp;
