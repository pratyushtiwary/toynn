...
class Sigmoid extends ActivationFunction {
    ...
    calculate(x) {
        const result = x.map((e) => 1 / (1 + Math.pow(Math.E, -e)));
...
class Relu extends ActivationFunction {
    ...
    calculate(x) {
        const result = x.map((e) => Math.max(0, e));
        return new narray_1.default(result);
    }
    toString() {
        return "relu";
...
class LeakyRelu extends ActivationFunction {
    ...
    calculate(x) {
        const result = x.map((e) => Math.max(0.01 * e, e));
        return new narray_1.default(result);
    }
    toString() {
        return "leakyRelu";
...
class Linear extends ActivationFunction {
    ...
    calculate(x) {
        const result = x.map((e) => e * __classPrivateFieldGet(this, _Linear_a, "f"));
        return new narray_1.default(result);
    }
    toString() {
