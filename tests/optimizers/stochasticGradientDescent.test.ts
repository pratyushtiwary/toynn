...
describe('StochasticGradientDescent tests', () => {
    test('StochasticGradientDescent', () => {
        const optimizer = new optimizers.StochasticGradientDescent();

        try {
            new optimizers.StochasticGradientDescent({
                momentum: 5,
            });
        ...
        const { x: processedDatasetX, y: processedDatasetY } =
            optimizer.process(datasetX, datasetY);

        if (processedDatasetX instanceof DatasetSlice) {
            expect(processedDatasetX.toArray()).toEqual(datasetX.toArray());
            expect(processedDatasetX.toArray()).not.toContain(
                datasetX.toArray()
            );
        } else {
            expect(true).toBe(false);
        ...
        if (processedDatasetY instanceof DatasetSlice) {
            ...
            expect(processedDatasetY.toArray()).not.toContain(
                datasetY.toArray()
            );
        } else {
            expect(true).toBe(false);
